#!/usr/bin/env python3
"""
Apex Oracle Bot – Multi-Parameter Sweep Optimizer
=================================================
Performs a grid search across key strategy parameters:
  - Max Hold Hours (8h, 16h, 24h, 48h)
  - Stop Loss % (1%, 2%, 3%, 5%)
  - Profit Target % (2%, 3%, 5%, 8%)

Goal: Find a parameter set that avoids the drawdown killswitch and achieves positive return.
"""
...
def run_simulation(data, max_hold_h, stop_loss_pct, profit_target_pct):
    # Fixed parameters
    ...
    MAX_PORT = 2500.0
    MAX_POS = 3
    DD_STOP = -10.0      # Production default
    DAILY_STOP = -3.0    # Production default
    ATR_MULT = 2.0
    FEE = 0.001
    CD = 6
    HU = 0.55
    ...
    for bi, t in enumerate(times):
        ...
        # Max drawdown check (Permanent kill)
        ...
        # Daily stop check (Halt until next day)
        ...
        for sym, df_full in data.items():
            ...
            if hp:
                ...
                hb = bi - pos.bar  # hold bars (1 bar = 1 hour)
                ...
                if sig == "BUY" and oc < MAX_POS and not daily_halt:
                    # Sizing: use ATR
                    risk = eq * BASE_RISK
                    sd = av * ATR_MULT
...
if __name__ == "__main__":
    ...
    # Sweep grids
    hold_hours_grid = [8, 12, 24, 48]
    stop_loss_grid = [0.01, 0.02, 0.03, 0.05]
    profit_target_grid = [0.02, 0.03, 0.05, 0.08]

    print("\nStarting Parameter Sweep...")
    print(f"{'Hold H':>6s} | {'SL%':>5s} | {'PT%':>5s} | {'Return%':>8s} | {'MaxDD%':>7s} | {'WR%':>6s} | {'PF':>6s} | {'Trades':>6s} | {'Killed':>6s}")
    print("-" * 75)

    results = []
    for h in hold_hours_grid:
        for sl in stop_loss_grid:
            for pt in profit_target_grid:
                res = run_simulation(data, h, sl, pt)
                res["h"] = h
                res["sl"] = sl
                res["pt"] = pt
                results.append(res)
                killed_str = "YES" if res["killed"] else "no"
                print(f"{h:>6d} | {sl*100:>4.1f}% | {pt*100:>4.1f}% | {res['ret']:>+7.2f}% | {res['mdd']:>6.2f}% | {res['wr']:>5.1f}% | {res['pf']:>5.2f} | {res['n']:>6d} | {killed_str:>6s}")

    print("\n" + "="*80)
    print("  TOP 5 CONFIGURATIONS BY RETURN (That survived without getting killed):")
    print("="*80)
    survived = [r for r in results if not r["killed"]]
    if not survived:
        print("  None of the configurations survived! (All hit the -5.0% drawdown stop).")
        # Sort all by return anyway
        survived = results
    
    survived = sorted(survived, key=lambda x: x["ret"], reverse=True)
    for i, r in enumerate(survived[:5]):
        print(f"  #{i+1}: Hold={r['h']}h, SL={r['sl']*100:.1f}%, PT={r['pt']*100:.1f}% => Ret={r['ret']:+.2f}%, MaxDD={r['mdd']:.2f}%, WR={r['wr']:.1f}%, PF={r['pf']:.2f}, Trades={r['n']}")
