...
trailing_peak = {}      # symbol -> highest price seen since trailing stop activated
start_equity = None
...
async def execute_trade(symbol: str, side: str, qty: float, price_estimate: float) -> bool:
    ...
    try:
        ...
        logged_ok = log_trade(symbol=symbol, side=side, quantity=clean_qty, price=filled_price,
                               pnl=realized_pnl, order_id=order_id)
        if not logged_ok:
            # The order filled on the exchange but is NOT recorded anywhere.
            # This is worse than an order failure -- the bot's real position
            # and the dashboard's tracked position have now silently
            # diverged. Page/alert immediately rather than continue quietly.
            logger.critical(
        ...
        try:
            ...
            for symbol in config.SYMBOLS:
                ...
                if has_position:
                    ...
                    # Once in profit beyond the activation threshold, track the
                    # highest price seen and exit if price retraces by the
                    # configured trailing distance. Resets when flat.
                    if config.TRAILING_STOP_ENABLED:
                        ...
                        if peak is not None and current_price <= peak * (1.0 - config.TRAILING_DISTANCE_PCT):
