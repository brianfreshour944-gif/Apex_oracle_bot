...
def get_recent_performance(bot_name='apex_oracle_bot', limit=50) -> float:
    ...
    try:
        ...
        for sym, t_list in symbol_trades.items():
            ...
            for t in t_list:
                side = t['side'].upper()
                # db.query_recent_trades returns 'quantity'; tolerate both names.
...
def check_account_killswitches(equity: float, starting_equity: float, bot_name='apex_oracle_bot') -> tuple:
    ...
    # 1. Lifetime Drawdown check (uses the lifetime baseline, set once at
    # first launch and never overwritten).
    ...
    # 2. Daily Loss Limit check (uses the daily baseline, reset each UTC
    # midnight -- a distinct column from the lifetime baseline so the two
    # mechanisms do not clobber each other).
    try:
        with db.engine.connect() as conn:
            result = conn.execute(
                db.text("SELECT daily_starting_equity FROM bot_status WHERE bot_name = :bot_name"),
                {"bot_name": bot_name}
            ).first()
